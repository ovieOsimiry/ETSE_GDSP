/*****************************************************************************
* Filename:          C:\FPGA\Internship_Jobs\Codes\Virtex6_MUL_IP_CORE2/drivers/matrix_mul_ip_core_s_int_g_v1_00_a/src/matrix_mul_ip_core_s_int_g_selftest.c
* Version:           1.00.a
* Description:       Contains a diagnostic self-test function for the matrix_mul_ip_core_s_int_g driver
...
#define MATRIX_MUL_IP_CORE_S_INT_G_SELFTEST_BUFSIZE  4 /* Size of buffer (n bytes) is fixed to 4 for lite Master */
#define MATRIX_MUL_IP_CORE_S_INT_G_USER_NUM_REG  4/* Number of registers in slave */

/************************** Variable Definitions ****************************/

static Xuint8 __attribute__((aligned (128))) SrcBuffer[MATRIX_MUL_IP_CORE_S_INT_G_SELFTEST_BUFSIZE];   /* Source buffer      */
static Xuint8 __attribute__((aligned (128))) DstBuffer[MATRIX_MUL_IP_CORE_S_INT_G_SELFTEST_BUFSIZE];   /* Destination buffer */
...
XStatus MATRIX_MUL_IP_CORE_S_INT_G_SelfTest(void * baseaddr_p)
{
  ...
  for (write_loop_index = 0 ; write_loop_index < MATRIX_MUL_IP_CORE_S_INT_G_USER_NUM_REG; write_loop_index++)
    MATRIX_MUL_IP_CORE_S_INT_G_mWriteSlaveReg0 (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0 ; read_loop_index < MATRIX_MUL_IP_CORE_S_INT_G_USER_NUM_REG; read_loop_index++)
    if ( MATRIX_MUL_IP_CORE_S_INT_G_mReadSlaveReg0 (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
  ...
  if ( (MATRIX_MUL_IP_CORE_S_INT_G_mReadSlaveReg0 (baseaddr, 0)) != 0x0){
  ...
  * Master */
  ...
  for ( Index = 0; Index < MATRIX_MUL_IP_CORE_S_INT_G_SELFTEST_BUFSIZE; Index++ )
  {
    SrcBuffer[Index] = Index;
    DstBuffer[Index] = 0;
  }
  xil_printf("   - start user logic master module to receive one word from the source\n\r");
  MATRIX_MUL_IP_CORE_S_INT_G_MasterRecvWord(baseaddr, (Xuint32) SrcBuffer);
  while ( ! MATRIX_MUL_IP_CORE_S_INT_G_mMasterDone(baseaddr) ) {}
  xil_printf("   - transfer completed\n\r");
  xil_printf("   - start user logic master module to send onw word to the destination\n\r");
  MATRIX_MUL_IP_CORE_S_INT_G_MasterSendWord(baseaddr, (Xuint32) DstBuffer);
  while ( ! MATRIX_MUL_IP_CORE_S_INT_G_mMasterDone(baseaddr) ) {}
  xil_printf("   - transfer completed\n\r");
